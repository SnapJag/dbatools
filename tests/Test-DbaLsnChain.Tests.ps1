...
. ([IO.Path]::Combine(([string]$PSScriptRoot).Trim("tests"), 'src\private\functions', 'Test-DbaLsnChain.ps1'))

Describe "$commandname Unit Tests" -Tag 'UnitTests' {
    Context "Validate parameters" {
        [array]$knownParameters = 'FilteredRestoreFiles', 'Continue', 'EnableException'
